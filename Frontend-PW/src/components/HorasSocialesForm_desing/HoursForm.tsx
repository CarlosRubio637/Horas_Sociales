...
const HoursForm = () =>{
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if(!formData.acceptedTerms){
            setErrors({acceptTerms: 'Debes aceptar los términos y condiciones para enviar el formulario'});
            return;
        }
    ...
    const generatePDF = async () => {
        ...
        try {
            ...
            pdf.text(`Estudiante: ${formData.name} - Carnet: ${formData.studentId}`, 10, 1);

            pdf.text(`Fecha: ${new Date().toLocaleDateString()} - Hora: ${new Date().toLocaleTimeString}`, 10,15);

            pdf.save(`comprobante-servicio-social-${formData.studentId}.pdf`);

            alert('¡Solisitud enviada! Y se ha descargado un comprobante de tu inscripción')
            ...
            alert('Error al generar el comprobante de inscripción. Por favor intenta nuevamente.');
        }
    };
    ...
    return(
        ...
        <form  onSubmit={handleSubmit} className="hours-form">
            <div className="form-group">
                ...
                value={formData.name} onChange={handleChange} placeholder="Nombre completo" required/>
            </div>

            <div className="form-group">
                <label htmlFor="email">Correo electrónico:</label>
                <input type="email" id="email" name="email" 
                value={formData.email} onChange={handleChange} placeholder="carnet@uca.edu.sv" required/>    
            </div>

            <div className="form-group">
                <label htmlFor="phone">Teléfono:</label>
                <input type="tel" id="phone" name="phone" 
                value={formData.phone} onChange={handleChange} placeholder="0000-0000" required/>
            </div>
            ...
            <div className="form-group">
                ...
                onChange={handleChange} rows={4} placeholder="Explica tu motivo de inscripción" required></textarea>
            </div>
            ...
            <div className="form-warning">
                <h2 className="form-title">Importante</h2>
                    ...
                    <p>
                        Es obligatorio que despues de haber reportado tus horas sociales y haber cumplido con el total de 
                        seisientas horas deberas crear un informe final de tu servicio social estudiantil, sin este no se tomara
                        como completado tu servicio social, las indicaciones puedes encontrarlas en los siguientes enlaces:
                    </p>
            ...
            <div className="form-group checkbox-group">
                <input  className="check-conditions" type="checkbox" 
                id="acceptedTerms" name="acceptedTerms" checked={formData.acceptedTerms} onChange={handleChange}/>
                <label className="check-info" htmlFor="checkbox">
                    He leído y acepto las responsabilidades y condiciones que conyeva realizar mi servicio social
                </label>
                {errors.acceptTerms && (<span className="error-message">{errors.acceptTerms}</span>)}
            </div>

            <div className="form-actions">
                <button type="submit" className="submit-btn" disabled={!formData.acceptedTerms}>Envia tu solicitud</button>
            </div>
